...
var LibraryWebSocket = {
	...
	WebSocketConnect: function(instanceId) {
		...
		// JSON 텍스트 메시지를 위해 binaryType 설정 제거
		...
		instance.ws.onmessage = function(ev) {
			...
			// 🔍 상세한 디버그 로그 추가
			console.log("[JSLIB WebSocket] === MESSAGE DEBUG START ===");
			console.log("[JSLIB WebSocket] ev.data type:", typeof ev.data);
			console.log("[JSLIB WebSocket] ev.data:", ev.data);
			console.log("[JSLIB WebSocket] ev.data length:", ev.data ? ev.data.length : 'null');
			console.log("[JSLIB WebSocket] instanceof ArrayBuffer:", ev.data instanceof ArrayBuffer);
			console.log("[JSLIB WebSocket] === MESSAGE DEBUG END ===");
			...
			if (ev.data instanceof ArrayBuffer) {
				console.log("[JSLIB WebSocket] 🔄 Processing as ArrayBuffer");
				var dataBuffer = new Uint8Array(ev.data);
				console.log("[JSLIB WebSocket] ArrayBuffer length:", dataBuffer.length);
				
				var buffer = _malloc(dataBuffer.length);
				HEAPU8.set(dataBuffer, buffer);

				try {
					console.log("[JSLIB WebSocket] ArrayBuffer callback - instanceId:", instanceId, "buffer:", buffer, "length:", dataBuffer.length);
					if (Module.dynCall_viii) {
				...
				console.log("[JSLIB WebSocket] 🔄 Processing as String");
				console.log("[JSLIB WebSocket] String content:", ev.data);
				
				// UTF-8 인코딩을 위한 TextEncoder 사용
				var encoder = new TextEncoder();
				var dataBuffer = encoder.encode(ev.data);
				
				console.log("[JSLIB WebSocket] UTF-8 encoded bytes, length:", dataBuffer.length);
				var buffer = _malloc(dataBuffer.length);
				HEAPU8.set(dataBuffer, buffer);
				try {
					console.log("[JSLIB WebSocket] ✅ Calling Unity OnMessage callback with", dataBuffer.length, "bytes");
					console.log("[JSLIB WebSocket] instanceId:", instanceId, "buffer:", buffer, "length:", dataBuffer.length);
					
					// Unity WebGL 호환성을 위한 다양한 호출 방식 시도
					if (Module.dynCall_viii) {
						console.log("[JSLIB WebSocket] Using Module.dynCall_viii");
						Module.dynCall_viii(webSocketState.onMessage, instanceId, buffer, dataBuffer.length);
					} else if (Module['dynCall_viii']) {
						console.log("[JSLIB WebSocket] Using Module['dynCall_viii']");
						Module['dynCall_viii'](webSocketState.onMessage, instanceId, buffer, dataBuffer.length);
					} else {
						console.log("[JSLIB WebSocket] Using direct call");
						Runtime.dynCall('viii', webSocketState.onMessage, [instanceId, buffer, dataBuffer.length]);
					}
				} finally {
					_free(buffer);
